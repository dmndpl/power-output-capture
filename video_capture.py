...
class VideoCapture():
    ...
    def _save_crop_factor(self):
        ...
        while True:
            ...
            if self.roi_selected:
                cv2.rectangle(img_display, self.roi_start,
                              self.roi_end, (0, 255, 0), 2)
            ...
            # Press 'c' to confirm and store the ROI coordinates
                ...
                x1, y1 = min(self.roi_start[0], self.roi_end[0]), min(
                    self.roi_start[1], self.roi_end[1])
                x2, y2 = max(self.roi_start[0], self.roi_end[0]), max(
                    self.roi_start[1], self.roi_end[1])
                ...
                # Wait for any key press before closing the ROI window
                cv2.waitKey(0)
    ...
    def read_frame_local(self, seconds_to_skip=SECONDS_TO_SKIP):
        if not self.cap:
            ...
            logging.info(
                "Video capture has a total of %s frames at %s frames per second", self.total_frames, self.fps)
